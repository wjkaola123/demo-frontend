...
function Modal({ children, onClose }: { children: ReactNode; onClose: () => void }) {
  ...
  return createPortal(
    ...
    >
      <div className="bg-white rounded-lg shadow-xl p-6 min-w-80 max-w-md relative">
        <button
          className="absolute top-2 right-3 text-gray-400 hover:text-gray-700 text-xl leading-none cursor-pointer"
          onClick={onClose}
        >
          &times;
        </button>
  ...
  return (
    <div className="border-2 border-blue-400 rounded-lg p-4">
      ...
      <p className="text-sm text-gray-600 mb-3">
        点击下方按钮打开模态框。模态框 DOM 被渲染在 <code className="bg-gray-200 px-1 rounded">document.body</code>{' '}
      ...
      {open && (
        <Modal onClose={() => setOpen(false)}>
          ...
          <p className="text-gray-600 text-sm mb-4">
            这个模态框通过 <code className="bg-gray-200 px-1 rounded">createPortal</code> 渲染到
            <code className="bg-gray-200 px-1 rounded">document.body</code> 下。
            检查 DOM 结构可以看到它不在当前组件的 DOM 嵌套中，但 React 事件系统仍能正常运作。
          </p>
          <div className="flex justify-end gap-2">
            <button
              className="bg-gray-200 text-gray-700 px-3 py-1 rounded cursor-pointer hover:bg-gray-300"
...
function Tooltip({ children, content }: { children: ReactNode; content: string }) {
  ...
  return (
    <>
      ...
      {show && createPortal(
        <div
          className="fixed z-50 px-2 py-1 bg-gray-800 text-white text-xs rounded whitespace-nowrap pointer-events-none"
...
function TooltipDemo() {
  return (
    <div className="border-2 border-green-400 rounded-lg p-4">
      ...
      <p className="text-sm text-gray-600 mb-3">
        将 tooltip 通过 portal 渲染到 <code className="bg-gray-200 px-1 rounded">document.body</code>，
        避免被父容器的 <code className="bg-gray-200 px-1 rounded">overflow: hidden</code> 裁切。
      </p>
      <p className="text-sm">
        将鼠标悬停在
      ...
      <div className="mt-3 overflow-hidden border border-red-300 rounded p-2 text-sm text-gray-500">
    ...
    <div className="p-8 max-w-2xl">
      ...
      <div className="mt-4 p-4 bg-gray-100 rounded text-sm">
        ...
        <ul className="list-disc pl-4 space-y-1">
          <li>语法: <code className="bg-gray-200 px-1 rounded">createPortal(reactNode, domNode)</code></li>
