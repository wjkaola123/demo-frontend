...
const Item = memo(function Item({ id, label, onRemove }: ItemProps) {
  ...
  return (
    <div ref={ref} className="flex items-center justify-between px-3 py-2 border rounded bg-card">
      <span>{label}</span>
      <button
  ...
  return (
    <div className="border-2 border-green-400 rounded-lg p-4 flex flex-col gap-3">
      ...
      <p className="text-sm text-muted-foreground">
        <code className="bg-muted px-1 rounded">handleRemove</code> 被 useCallback 缓存，引用不变，
      ...
      {items.length === 0 && <p className="text-muted-foreground text-sm">所有商品已删除</p>}
    </div>
  )
}
...
function WithoutUseCallback() {
  ...
  return (
    <div className="border-2 border-red-400 rounded-lg p-4 flex flex-col gap-3">
      ...
      <p className="text-sm text-muted-foreground">
        每次渲染创建新的 <code className="bg-muted px-1 rounded">handleRemove</code>，
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="mt-6 p-4 bg-muted rounded text-sm">
        ...
        <ul className="list-disc pl-4 space-y-1">
          <li>
            <code className="bg-muted px-1 rounded">useCallback(fn, deps)</code> —
            仅当 deps 变化时返回新的函数引用，否则返回缓存的旧引用
          </li>
          <li>
            核心场景：将回调传给 <code className="bg-muted px-1 rounded">memo()</code> 包裹的子组件，
            避免因函数引用变化导致的无效重渲染
          </li>
          <li>
            等价于 <code className="bg-muted px-1 rounded">useMemo(() =&gt; fn, deps)</code>，
            useCallback 是专门缓存函数的语法糖
          </li>
          <li>deps 为空数组 `[]` 时函数只创建一次，内部引用的 state 永远是初始值（闭包陷阱）</li>
          <li>
            如函数内需读取最新 state，使用
            <code className="bg-muted px-1 rounded">setState(prev =&gt; ...)</code> 函数式更新避免依赖过时值
          </li>
